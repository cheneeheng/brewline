# EX-01 — Broken trace context at the broker
...
- **Time:** ~5 minutes.
- **Impact:** no downtime; recreating the three services drops in-flight requests.
  Fully reversible — the flag changes instrumentation only.
...
2. Find the **storefront** trace for your order and open the waterfall. The
   storefront-rooted trace now **ends at the `order.placed publish` span** — the
   `fulfillment.process` and `notification.notify` spans are no longer in it.

3. Change the **Service** dropdown to **fulfillment**, then **Find Traces**.
   Fulfillment (and notification) now appear as **separate root traces** with their own
   trace IDs.

**Verify:** the work still happens, but the trace is fragmented, and there is no error
anywhere. This is a *silent* failure, the same shape as the cross-language Go break
described in [Concepts](../concepts.md#cross-language-propagation-python--go).

**If it fails:**

- The async spans are still in the storefront trace → a container is running the old
  flag value. Compose reads `.env` only when it *creates* a container, so check each
  one and recreate any that still print `on`:

  ```bash
  docker compose -f deploy/docker-compose.yml exec fulfillment printenv BROKER_PROPAGATION
  ```

- No `fulfillment.process` spans anywhere → the consumer is not running, which is a
  different problem. Check `docker compose -f deploy/docker-compose.yml logs fulfillment`
  and the depth of the `fulfillment.order.placed` queue in the RabbitMQ UI
...
[Guide index](../index.md) · [EX-02 Metric cardinality explosion →](EX-02-metric-cardinality.md)
